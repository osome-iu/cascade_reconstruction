...
Output:
    - The output directories will contain subdirectories that indicate the creation parameters
...
# Change the directory to the location of this script, ensures relative paths work
...
DATA_FILE = "/data_volume/cascade_reconstruction/temp/cascade_records.parquet"
OUT_DIR = "/data_volume/cascade_reconstruction/pdi_midterm"

# Setting values for the power law
# GAMMA_VALUES = [0.25, 0.5, 0.75]
# ALPHA_VALUES = [1.1, 1.5, 2.0, 2.5, 3.0]
GAMMA_VALUES = [0.25]
ALPHA_VALUES = [1.1, 2.0, 3.0]
XMIN = 1
    ...
    for gamma in GAMMA_VALUES:
        ...
        for alpha in ALPHA_VALUES:
            ...
            for cas_num, cascade_id in enumerate(all_cascades, start=1):
                ...
                cascade_frame = cascade_frame.sort_values("timestamp", ascending=True)
                ...
                poten_edge_users = list(cascade_frame["user_id"])
                poten_edge_tstamps = list(cascade_frame["timestamp"])
                poten_edge_fcounts = np.array(cascade_frame["follower_count"])
